...
impl Segment {
    pub fn intersection(&self, r: &Ray) -> Option<DVec2> {
        // ax + by + c = 0
        let d = self.b - self.a;
        let (a1, b1, c1) = {
...
impl Scene {
    pub fn sample(&self, ray: &Ray) -> Option<HitData> {
        ...
        for segment in &self.segments {
            if let Some(pos) = segment.intersection(ray) {
                let d = ((pos - ray.origin) / ray.dir).x;
                if d > 0.0 && d < dist {
                    closest_color = segment.color;
                    closest = pos;
                    dist = d;
                }
            }
        ...
        Some(HitData { color: closest_color, dist, point: closest })
    }
}